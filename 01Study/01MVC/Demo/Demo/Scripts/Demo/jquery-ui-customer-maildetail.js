$.widget("custom.maildetail",
           {
               ...
               _create: function () {
                   ...
                   self._initVals();
                   self._initElement(self.options.folderId);
               },
               ...
               _initElement: function (updataData) {
                   var self = this;
                   this.element.addClass(self._css);
                   this._mailTable = $("<div class='restore-table'></div").appendTo(this.element);
                   this._pagation = $("<ul class='pagination maildetail-pagination'></ul>").appendTo(this.element);
                   self._folderId = updataData.data.folderId;
                   self._pageCount = updataData.data.pageCount || self._pageCount;
                   self._catalogJob = updateData.data.catalogJob;
                   self._ajaxUrl = updataData.url;
                   $.ajax({
                       type: "POST",
                       url: updataData.url,
               ...
               _addItemToTree: function (parentId, parentChildCount, childContainers) {
                   ...
                   $.each(childContainers, function (i, item) {
                       var childItem = Restore.Item.CreateItem(item.Id, item, 0, function (event, data) {
               ...
               _removeListen: function () {
                   var self = this;
                   self._mailTable.bind("click.ui.customer.maildetail");
               },
               ...
               _onPageChanged: function (lastPage, currentPage) {
                   ...
                   if (items) {
                       ...
                       $.ajax({
                           ...
                           data: { catalogJob: self._catalogJob, folderId: self._folderId, pageIndex: currentPage - 1, pageCount: self._pageCount },
                           success: function (data) {
                               self._renderTable(data.Mails);
                           }
                       });
