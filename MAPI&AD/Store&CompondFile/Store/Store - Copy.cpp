...
LPSTR ConvertUnicode2Ansi(LPWSTR unicodeStr,int &cbLen){
	bool isUsedDefaultChar;
	int cchWLen = wcslen(unicodeStr);

	cbLen = WideCharToMultiByte(CONVERT_CODE_PAGE, 0, unicodeStr, -1, NULL, 0, NULL, NULL);
...
#define FILEPATH L"D:\\12issue\\141583_Exchange\\01TestMail\\GenerateByCode\\Copy\\Garbage.msg"
//#define FILEPATH L"D:\\12issue\\141583_Exchange\\01TestMail\\GenerateByCode\\Copy\\Correct.msg"
int _tmain(int argc, _TCHAR* argv[]){
	OutputDebugString(FILEPATH);
	CCompondFileHelper compoundFileHelper;
	compoundFileHelper.DisplayAllInfo(FILEPATH);

}
#endif

#define MSG L"ceshi测试12";
void TestCode(){
	LPWSTR unicode = MSG;
	LPSTR ansi = ConvertUnicode2Ansi(unicode);

	size_t len = lstrlen(unicode);

	byte temp[255];
	
	memset(temp, 0, 255);
	int length = sizeof(wchar_t)*(len + 1);
	memcpy((void*)temp, (void *)unicode, length);
	printf("Unicode:\n");
	...
	int ansiLen = 0;
	LPSTR result = ConvertUnicode2Ansi(unicode, ansiLen);

	memset(temp, 0, 255);
	length = ansiLen;
	memcpy((void*)temp, (void *)result, length);
