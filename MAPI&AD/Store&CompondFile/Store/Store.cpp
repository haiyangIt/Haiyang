...
LPWSTR ConvertAnsi2Unicode(LPSTR ansiStr){
	//ansi to unicode
	int sLen = MultiByteToWideChar(CONVERT_CODE_PAGE, NULL, ansiStr, -1, NULL, 0);
	wchar_t* sUnicode = new wchar_t[sLen+1];
	memset((void *)sUnicode, 0, sizeof(wchar_t)*(sLen+1));
	MultiByteToWideChar(CONVERT_CODE_PAGE, NULL, ansiStr, strlen(ansiStr), sUnicode, sLen);
	return sUnicode;
}
...
void SaveMsg2File(){
	...
	LPWSTR receipt = L"xila@lhytest.com";
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	/*char* errorStr = "¥Æ¥¹¥È";
	char* normalSubjectStr = "Æ0¹0È0";
	wchar_t* normalWStr = L"テスト";

	char* normalW2AStr = NULL;
	wchar_t* errorA2WStr = NULL;
	wchar_t* normalSubjectA2WStr = NULL;

	errorA2WStr = ConvertAnsi2Unicode(errorStr);
	normalSubjectA2WStr = ConvertAnsi2Unicode(normalSubjectStr);
	normalW2AStr = ConvertUnicode2Ansi(normalWStr);

	printf_s("errorStr:%s\n", errorStr);
	wprintf_s(L"errorStr U:%s\n", errorA2WStr);

	printf_s("normalSubject A:%s\n", normalSubjectStr);
	wprintf_s(L"normalSubject U:%s\n", normalSubjectA2WStr);

	printf_s("normalWStr A:%s\n", normalW2AStr);
	wprintf_s(L"normalWStr U:%s\n", normalWStr);
