...
namespace SqlDbImpl.Model
{
    ...
    public class ItemLocationModel : IItemData
    {
        ...
        internal static string GetItemContainerName(string mailboxName, string folderIdMd5Str, int index)
        {
            return string.Format("{0}{3}{1}{3}{2}", ServiceContext.GetOrganizationPrefix(mailboxName), folderIdMd5Str, index, BlobDataAccess.DashChar);
        }

        internal static string GetItemContainerName(IItemData itemData, string mailboxName)
        {
            return GetItemContainerName(mailboxName, MD5Utility.ConvertToMd5(itemData.ParentFolderId), 0);
        }

        internal static string GetItemNextContainerName(string containerName, string mailboxName)
        {
