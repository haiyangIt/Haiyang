...
using Arcserve.Exchange.FastTransferUtil.CompoundFile.MsgStruct;
...
namespace Arcserve.Exchange.FastTransferUtil.CompoundFile
{
    public class CompoundFileUtil
    {
        ...
        public void ConvertBinToMsg(Stream binStream, Stream msgStream)
        {
            ILockBytes memory = null;
            try
            {
                binStream.Seek(0, SeekOrigin.Begin);
                BuildMsgInMemoryWithBin(binStream, out memory);
                var bytes = ReadAllByteInlockBytes(memory);
                msgStream.Write(bytes, 0, bytes.Length);
