    ...
    public class FTStreamReaderForPage : IFTStreamReader
    {
        ...
        public Item.PropertyTag ReadPropertyTag()
        {
            if (_reader.IsEnd)
            {
                if (!_ftPage.IsLastPage)
                {
                    var result = FTFactory.Instance.CreatePropertyTag(ReadUInt32(false));
                    return result;
                }
                else
                {
                    return Item.PropertyTag.Empty;
                }
            }
            else
            {
                return FTFactory.Instance.CreatePropertyTag(ReadUInt32(false));
            }
        }
    }
}
